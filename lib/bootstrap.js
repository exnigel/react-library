...
class Select extends react_1.default.Component {
    ...
    render() {
        ...
        return R("select", {
            ...
            className: (0, classnames_1.default)("block w-full bg-surface text-ink border border-border-strong rounded focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary disabled:opacity-65", this.props.size === "sm" ? "px-2 py-1 text-sm" : this.props.size === "lg" ? "px-4 py-2 text-lg" : "px-3 py-1.5 text-base"),
    ...
    render() {
        return R("ul", { className: "flex flex-wrap list-none p-0 m-0" }, lodash_1.default.map(this.props.pills, (pill) => {
            return R("li", { key: pill.id }, R("a", {
                ...
                className: (0, classnames_1.default)("inline-block px-4 py-2 rounded cursor-pointer no-underline", pill.id === this.props.activePill ? "bg-primary text-on-primary" : "text-primary hover:bg-border/40")
            }, pill.label));
        }));
    }
}
...
class Toggle extends react_1.default.Component {
    constructor() {
        ...
        this.renderOption = (option, index) => {
            ...
            const size = this.props.size == "xs" ? "sm" : this.props.size;
            const btnClasses = (0, classnames_1.default)("inline-block text-center align-middle cursor-pointer select-none border rounded transition-colors", size === "sm" ? "px-2 py-1 text-sm" : size === "lg" ? "px-4 py-2 text-lg" : "px-3 py-1.5 text-base", isSelected
...
function CollapsiblePanel(props) {
    ...
    return react_1.default.createElement("div", { className: "border border-border rounded mb-4" },
        react_1.default.createElement("div", { className: "px-4 py-2 bg-bg border-b border-border" },
